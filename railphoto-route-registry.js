...
(function(){
  ...
  function render(){const routes=filtered(),summary=document.getElementById('rrSummary');if(summary)summary.innerHTML=`<div class="rr-card"><b>Маршрутов</b><div style="font-size:20px">${routes.length}</div></div><div class="rr-card"><b>Городских</b><div style="font-size:20px">${routes.filter(r=>r.color==='red').length}</div></div><div class="rr-card"><b>Грузовых</b><div style="font-size:20px">${routes.filter(r=>r.color==='black').length}</div></div><div class="rr-card"><b>УЖД</b><div style="font-size:20px">${routes.filter(r=>r.color==='green').length}</div></div>`;const body=document.getElementById('rrBody');if(!body)return;body.innerHTML=routes.map(r=>{const c=LINE_COLORS[r.color]||LINE_COLORS.blue;const isFreight=r.type==='Грузовой'||r.color==='black';const cls=`rr-line-row rr-line-${r.color}${isFreight?' rr-freight-row':''}`;return `<tr class="${cls}" style="color:inherit!important;box-shadow:inset 6px 0 0 ${c.color};"><td style="width:8px;padding:0;background:${c.color}!important;" title="${esc(c.label)}"></td><td><b>${esc(r.num)}</b></td><td>${esc(r.from||'—')}</td><td>${esc(r.to||'—')}</td><td>${esc(r.type||'—')}</td><td>${esc(r.arr||'—')}</td><td>${esc(r.dep||'—')}</td><td>${esc(r.consist||'—')}</td><td>${esc(r.trains||'—')}</td><td>${esc(r.wagons||'—')}</td><td>${esc(r.cars||'—')}</td><td>${esc(r.notes||'—')}</td><td><button class="btn-secondary btn-sm" data-edit="${esc(r.id)}">✏️</button> <button class="btn-danger btn-sm" data-del="${esc(r.id)}">🗑</button></td></tr>`;}).join('')||'<tr><td colspan="13" style="text-align:center;padding:16px">Маршрутов пока нет</td></tr>';body.querySelectorAll('tr.rr-freight-row td, tr.rr-freight-row td *').forEach(el=>el.style.setProperty('color','#ffffff','important'));
  }
